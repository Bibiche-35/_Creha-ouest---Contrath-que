...
class ConventionManager extends DB
{
    ...
    public function readConventionsClients()
    {
        ...
        $req = $db->query('SELECT client.siret_client, denomination_client, boolean_convention, durée_mois_conv, montant_annuel_conv, commentaire_conv, lien_conv, DATE_FORMAT(date_debut_conv, \'%d/%m/%Y\') AS date_debut_conv_fr, DATE_FORMAT(date_fin_conv, \'%d/%m/%Y\') AS date_fin_conv_fr, date_debut_conv, date_fin_conv, id_type_client, id_type, denomination_type FROM client, type_client, informations_convention WHERE boolean_convention = 1 AND client.id_type_client = type_client.id_type AND informations_convention.siret_info_convention = client.siret_client');
